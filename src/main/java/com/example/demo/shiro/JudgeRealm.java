...
public class JudgeRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        logger.info(gser.getUsername()+"登入进行授权");
        /*创建 权限字符列表 和 角色字符列表*/
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken Token) throws AuthenticationException {
        ...
        /*验证登录*/
        CurrentUser teacher = teacherMapper.loginCheck(username);
        //System.out.println(teacher);
        if (teacher==null){
            teacher = teacherMapper.loginCheckCJ(username);
            logger.info("course_judge-->sign in");
        }
        return new SimpleAuthenticationInfo(teacher,teacher.getPwd(),this.getClass().getName());
    }
}
